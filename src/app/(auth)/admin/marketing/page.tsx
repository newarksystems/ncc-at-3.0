...
import CallRatesPerHour from '@/components/callratesperhour';
...
import { MdPhoneCallback, MdOutlineSupportAgent } from 'react-icons/md';
...
const MarketingAdminDashboard: React.FC = () => {
  const { user: authUser, isAuthenticated, loading: authLoading } = useAuth();
  const router = useRouter();

  useEffect(() => {
    // Redirect if user is not authenticated or not a marketing admin
    if (!authLoading && isAuthenticated) {
      if (!authUser || (authUser.role !== 'admin' && authUser.role !== 'super-admin') || 
          (authUser.role === 'admin' && authUser.designation !== 'marketing-admin')) {
  ...
  // Show not authorized if user is authenticated but not a marketing admin
  if (isAuthenticated && 
      (!authUser || (authUser.role !== 'admin' && authUser.role !== 'super-admin') || 
       (authUser.role === 'admin' && authUser.designation !== 'marketing-admin'))) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#1a2332]">
        <div className="text-white">Access denied. Marketing admin privileges required.</div>
      </div>
    );
  }
