...
import { RefreshCcw, Pause, Users, Settings, MoreHorizontal } from 'lucide-react';
...
const CallCenterAdminDashboard: React.FC = () => {
  ...
  useEffect(() => {
    // Redirect if user is not authenticated or not a call-center admin
    if (!authLoading && isAuthenticated) {
      if (!authUser || (authUser.role !== 'admin' && authUser.role !== 'super-admin') || 
          (authUser.role === 'admin' && authUser.designation !== 'call-center-admin')) {
        if (authUser?.role === 'admin' && authUser.designation) {
          switch(authUser.designation) {
            ...
            case 'compliance-admin':
              ...
              break;
            default:
              // Default to call-center dashboard for call-center admin
              break;
          }
  ...
  // Show not authorized if user is authenticated but not a call-center admin
  if (isAuthenticated && 
      (!authUser || (authUser.role !== 'admin' && authUser.role !== 'super-admin') || 
       (authUser.role === 'admin' && authUser.designation !== 'call-center-admin'))) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#1a2332]">
        <div className="text-white">Access denied. Call center admin privileges required.</div>
      </div>
    );
  }
  ...
  if (loading) {
    return (
      <DashboardLayout title="Call Center Dashboard">
